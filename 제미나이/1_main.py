...
class ExportRequest(BaseModel):
    ...
    resolution: str = Field("original", pattern="^(original|720p|1080p|1440p|4k)$")
...
# 디버그용 — 서버가 로드한 코드 버전 확인 (필요시 활성화)
# @app.get("/api/debug/code-version")
# async def debug_code_version():
#     import inspect
#     source = inspect.getsource(video_processor.burn_subtitles)
#     return {"drawtext": "drawtext" in source, "subtitles_filter": "subtitles=" in source}
...
async def merge_export(request: MergeExportRequest) -> dict:
    ...
    try:
        ...
        # Step D: 최종 내보내기 (format/quality/resolution)
        gen = video_processor.export_video(
            final_path,
            quality=request.quality,
            format_type=request.format,
