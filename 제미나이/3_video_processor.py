...
class VideoProcessor:
    ...
    def burn_subtitles(
        ...
        try:
            ...
            # FFmpeg drawtext에서 콜론 이스케이프: C\:/Windows/...
            font_candidates = [
            ...
            for fc in font_candidates:
                if os.path.exists(fc):
                    ...
                    break
            if raw_font is None:
                raw_font = "C:/Windows/Fonts/malgun.ttf"  # 기본값

            # FFmpeg drawtext에서 경로의 콜론과 백슬래시 이스케이프
            font_path = raw_font.replace("\\", "/").replace(":", "\\:")
            ...
            # 디버그: 실제 FFmpeg 명령어 출력
            print(f"[burn_subtitles] Font: {font_path}")
            print(f"[burn_subtitles] Subtitles count: {len(subtitles)}")
            print(f"[burn_subtitles] CMD: ffmpeg -y -i {path} -vf <{len(drawtext_filters)} drawtext filters> ... {output_path}")
            ...
            if result.returncode != 0:
                print(f"[burn_subtitles] STDERR: {result.stderr[-1000:]}")
                raise RuntimeError(
    ...
    def export_video(
        ...
        """
        Export final video with quality and resolution settings.
        ...
        Args:
            ...
            resolution: Target resolution — "original", "1080p", "1440p", "4k"
        ...
        try:
            ...
            # 해상도 맵: 높이 기준 (너비는 -2로 자동 계산하여 짝수 보장)
            resolution_map = {
                "original": None,
                "720p": 720,
                "1080p": 1080,
            ...
            target_h = resolution_map.get(resolution)
            ...
            if target_h:
                ...
                if info.height != target_h:
                    # scale=-2:1080 → 너비 자동 계산 (짝수), 높이 1080
                    scale_filter = f"scale=-2:{target_h}"
                    print(f"[export] Upscale: {info.width}x{info.height} → ?x{target_h}")
            ...
            if format_type in ["horizontal", "both"]:
                yield {
                    "status": "exporting",
                    "progress": 10,
                    "message": f"가로(16:9) 내보내기 중... {resolution if target_h else '원본 해상도'}",
                }
            ...
            if format_type in ["vertical", "both"]:
                ...
                # 세로 변환: 블러 배경 + 중앙 오버레이
                # 해상도 옵션 반영: 기본 1080x1920, 1440p→1440x2560, 4K→2160x3840
                v_info = self.get_video_info(path)
                vert_sizes = {
                    720: (720, 1280),
                    1080: (1080, 1920),
                    1440: (1440, 2560),
                ...
                vw, vh = vert_sizes.get(target_h, (1080, 1920)) if target_h else (1080, 1920)
                vf_filter = (
                    f"[0:v]scale={vw}:{vh}:force_original_aspect_ratio=increase,"
                    f"crop={vw}:{vh},boxblur=20:5[bg];"
